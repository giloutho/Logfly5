    ...
    private boolean genScoreDetails() {
        ...
        try {            
            ...
            for (int i = 1; i < LgTb; i++) {
                ...
                if (i == 1) {                    
                    if ( !KmlTrace.getScore_Triangle()) {                        
                        dDist = (org.logfly.geoutils.trigo.CoordDistance(currPoint.Latitude,currPoint.Longitude,pointB.Latitude,pointB.Longitude))/1000;     
            ...
            if (KmlTrace.getScore_Triangle()) {
                ...
                pointIGC pointB = KmlTrace.Tb_Calcul.get(KmlTrace.Score_Tb_Balises.get(LgTb-1));
                dDist = (org.logfly.geoutils.trigo.CoordDistance(pointB.Latitude,pointB.Longitude,pointA.Latitude,pointA.Longitude))/1000;  
                sDist = String.format(Locale.ROOT,"%5.2f",dDist);       
                kmlScoreDetails.append("<TD nowrap>").append(sDist).append(" km</TD>");
                PartDist = (dDist / KmlTrace.getScore_Route_Km()) * 100;
                kmlScoreDetails.append("<TD  align=\"right\">").append(String.format(Locale.ROOT,"%3.2f",PartDist)).append("%</TD></TR>").append(RC); 
                ...
                dDist = (org.logfly.geoutils.trigo.CoordDistance(pointA.Latitude,pointA.Longitude,pointB.Latitude,pointB.Longitude))/1000;  
                sDist = String.format(Locale.ROOT,"%5.2f",dDist);                    
                kmlScoreDetails.append("<TD nowrap>").append(sDist).append(" km</TD>");
    ...
    */
    private String codeBalise(int IdxCurr, String nCurr,int IdxPcdt, String nPcdt, int IdxSvt, String nSvt) {
        ...
        if (IdxPcdt > 0) {
            ...
            dDist = (org.logfly.geoutils.trigo.CoordDistance(currPoint.Latitude,currPoint.Longitude,pointPcdt.Latitude,pointPcdt.Longitude))/1000;  
            sDist = String.format(Locale.ROOT,"%5.2f",dDist);       
        ...
        if (IdxSvt > 0) {
            ...
            dDist = (org.logfly.geoutils.trigo.CoordDistance(pointSvt.Latitude,pointSvt.Longitude,currPoint.Latitude,currPoint.Longitude))/1000; 
            sDist = String.format(Locale.ROOT,"%5.2f",dDist); 
